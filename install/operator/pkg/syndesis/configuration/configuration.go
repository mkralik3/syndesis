...
func ApiVersion(apiClient kubernetes.Interface) (string, error) {
	if apiClient == nil {
		return "<Version not determined>", nil
	}

	info, err := apiClient.Discovery().ServerVersion()
	if err != nil {
		return "Failed to discover server version", err
	}
...
func (config *Config) CheckRouteHostname() error {
	if config.ApiServer.Openshift {
		...
		// but k8 platforms will not so check the CR has provided one
		//
		return nil
	}
...
func GetProperties(ctx context.Context, file string, clientTools *clienttools.ClientTools, syndesis *v1beta1.Syndesis) (*Config, error) {
	...
	if v, err := ApiVersion(apiClient); err != nil {
		return nil, err
	} else {
		configuration.ApiServer.Version = v
	}

	if os, err := IsPlatformOpenshift(apiClient); err != nil {
		return nil, err
	} else {
		configuration.ApiServer.Openshift = os
	}
